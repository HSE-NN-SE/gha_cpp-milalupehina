...
- **nextPrime()** -  возвращение простого числа, расположенного следом за установленным
- **getValue()** - возвращение установленного числа.
...
- **main.cpp** - домострационная программы для работы с классом **Prime**.

Файл `Prime.h` помещаются в каталог `include`
Файлы `Prime.cpp, main.cpp` помещаются в каталог `src` 
